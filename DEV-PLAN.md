...
| Channel | 方向 | 参数类型 | 返回值类型 | 说明 |
|---------|------|---------|-----------|------|
| `config:get-resources` | R→M | `{ type: 'skills'\|'hooks'\|'plans'\|'tasks'\|'mcp' }` | `Resource[]` | 获取资源列表 |
...
| A4: CSS Grid 动态布局（1=全屏，2=对半，3=三等分，4=2x2，5=上3下2，6=3x2，7+=ceil(sqrt(n))列） | Renderer | A1 | 2天 |
| A5: 终端进程生命周期管理（状态机 + 状态点颜色/动画映射） | Main + Renderer | A2 | 1.5天 |
...
| B3: 3D 视觉效果（hover 倾斜 ±4°、光泽层、聚焦动画、状态点呼吸脉冲） | Renderer | B1 | 0.5天 |
...
| 任务 | 涉及层 | 前置依赖 | 工期 |
|------|--------|----------|------|
| D1: JSONL 解析器（流式逐行读取、格式错误跳过、不完整末尾行忽略） | 数据层 | 无 | 1天 |
| D2: 双源读取逻辑（CC 原始文件优先 → Muxvo 镜像兜底） | 数据层 | D1 | 0.5天 |
| D3: 聊天历史同步模块（启动全量扫描 + 运行中增量同步） | Main + 数据层 | D1 | 2天 |
| D4: 三栏邮件客户端布局（项目列表 220px + 会话列表 340px + 详情 flex） | Renderer | D1 | 1.5天 |
...
| 任务 | 涉及层 | 前置依赖 | 工期 |
|------|--------|----------|------|
| G1: Markdown 渲染引擎（markdown-it/marked + CommonMark + GFM） | Renderer | 无 | 1.5天 |
...
| # | 任务 | 涉及层 | 前置依赖 | 预估 |
|---|------|--------|---------|------|
| N2-1 | 定义聚合源统一数据模型（UnifiedPackage 接口） | Domain | 无 | 0.5d |
...
| SS-3 | HTML 生成引擎（合并评分 + SKILL.md → 静态 HTML） | Service | SS-1, SS-2, SR | 2d |
...
```typescript
// Main → Renderer
'marketplace:packages-loaded'   // 聚合数据加载完成
'marketplace:install-progress'  // 安装进度
'marketplace:score-result'      // 评分结果
'marketplace:publish-result'    // 发布结果
'marketplace:update-available'  // 更新可用通知

// Renderer → Main
'marketplace:load-packages'     // 加载聚合数据
'marketplace:install-package'   // 安装包
'marketplace:uninstall-package' // 卸载包
'marketplace:trigger-score'     // 触发评分
'marketplace:publish-skill'     // 发布 Skill
'marketplace:check-updates'     // 检查更新
'marketplace:oauth-login'       // GitHub OAuth 登录
'marketplace:oauth-logout'      // 登出
