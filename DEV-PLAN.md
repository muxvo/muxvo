...
| `terminal:write` | R→M (send) | `{ id: string, data: string }` | void | 向 PTY 写入数据（同时用于 V2 评分指令写入） |
...
| `terminal:get-state` | R→M | `{ id: string }` | `TerminalInfo` | 获取单个终端状态（供 V2 AI 评分检测 CC 终端状态使用） |
...
| `app:memory-warning` | M→R | `{ usageMB: number, threshold: number }` | - | 内存使用超阈值警告推送（memory-monitor.ts 每 60s 检查，超 2GB 时推送） |
| `app:detect-cli-tools` | R→M | void | `{ claude: boolean, codex: boolean, gemini: boolean }` | 检测系统 PATH 中已安装的 AI CLI 工具（首次引导流程使用，PRD §8.14） |
...
| `marketplace:install` | R→M | `{ name: string, source: string, type: 'skill'\|'hook', version?: string }` | `{ success: boolean }` | 安装包（version 可选，用于更新检测后安装特定版本） |
...
| `marketplace:packages-loaded` | M→R | `{ packages: AggregatedPackage[], source: string }` | - | 聚合数据加载完成推送（异步加载模式下，各源数据就绪时逐批推送） |
| `marketplace:update-available` | M→R | `{ packages: UpdateInfo[] }` | - | 发现可更新包推送（后台定时检测发现更新时推送） |

### 2.7 AI 评分域 (score:*) — V2-P1

| Channel | 方向 | 参数类型 | 返回值类型 | 说明 |
|---------|------|---------|-----------|------|
| `score:run` | R→M | `{ skillDirName: string, includeAnalytics?: boolean }` | `SkillScore` | 触发 AI 评分（includeAnalytics 控制是否纳入使用统计数据，需用户授权） |
| `score:check-scorer` | R→M | void | `{ installed: boolean, version?: string }` | 检查评分 Skill（muxvo-skill-scorer）是否已安装 |
...
| `score:result` | M→R | `{ skillDirName: string, score: SkillScore }` | - | 评分最终结果推送（评分完成后异步通知 Renderer） |
...
| `showcase:publish` | R→M | `{ skillDirName: string, details?: { problem: string, solution: string, screenshots: string[] }, securityChecked: boolean }` | `{ url: string }` | 发布到 GitHub Pages（details 为用户填写的详情，securityChecked 标识是否已通过安全检查） |
| `showcase:unpublish` | R→M | `{ skillDirName: string }` | `{ success: boolean }` | 下线展示页 |
| `showcase:publish-result` | M→R | `{ skillDirName: string, success: boolean, url?: string, error?: string }` | - | 发布结果异步通知（发布耗时较长时，通过推送通知 Renderer 最终结果） |

### 2.9 认证域 (auth:*) — V2-P2

| Channel | 方向 | 参数类型 | 返回值类型 | 说明 |
|---------|------|---------|-----------|------|
| `auth:login-github` | R→M | void | `{ username: string, loggedIn: boolean }` | GitHub OAuth PKCE 登录（token 仅在 Main 进程通过 safeStorage 管理，Renderer 不接触 token） |
...
- Channel 名称采用 `域:动作` 格式（如 `terminal:create`、`score:run`）
- 包名统一使用 `@scope/name` namespace 格式（如 `@muxvo/skill-scorer`、`@muxvo/publisher`），V1 预埋此规范，V2 聚合源包名遵循此约定

### 2.12 统一 IPC 错误响应格式

所有 invoke 通道在失败时统一抛出以下格式的错误对象：

```typescript
interface IPCError {
  code: string;       // 错误码，格式为 "域.错误类型"（如 "terminal.spawn_failed"、"marketplace.download_failed"）
  message: string;    // 人类可读的错误描述
  details?: any;      // 可选的附加信息（如原始异常堆栈、失败路径等）
}
```

通用错误码示例：
- `terminal.spawn_failed` — PTY 创建失败（路径不存在/权限不足）
- `terminal.close_timeout` — 终端关闭超时（SIGINT 后仍未退出）
- `chat.parse_failed` — Session JSONL 解析失败
- `marketplace.download_failed` — 包下载失败
- `marketplace.install_failed` — 包安装失败（解压/权限/版本冲突）
- `score.cc_not_running` — CC 未运行或未就绪
- `score.parse_failed` — 评分结果 JSON 解析失败
- `showcase.publish_failed` — 发布失败（未登录/超时/名称冲突/网络中断）
- `auth.oauth_failed` — OAuth 流程失败
- `fs.permission_denied` — 文件系统权限不足
- `fs.not_found` — 文件或目录不存在

Main 进程 handler 实现时，所有 `ipcMain.handle` 中的异常应统一转换为此格式后抛出，Renderer 通过 `try/catch` 捕获。
...
**xterm.js scrollback buffer 动态缩减：**
- 非可见终端（非当前聚焦/显示的 tab）：`terminal.options.scrollback = 1000`
- 切回可见时恢复：`terminal.options.scrollback = 10000`
- 目的：减少多终端场景下的内存占用，避免后台终端无限累积 scrollback 数据
...
| 新建终端 | `⌘T` | `Ctrl+T` |
| 关闭当前终端 | `⌘W` | `Ctrl+W` |
| 聚焦第 N 个终端 | `⌘1~9` | `Ctrl+1~9` |
| 切换到上/下一个终端 | `⌘[` / `⌘]` | `Ctrl+[` / `Ctrl+]` |
| 返回平铺/关闭面板/关闭临时视图 | `Esc` | `Esc` |
| 搜索（上下文感知） | `⌘F` | `Ctrl+F` |
| 切换文件面板 | `⌘E` | `Ctrl+E` |
| 打开/关闭 Skill 浏览器 | `⌘⇧M` | `Ctrl+Shift+M` |
| 切换 Markdown 预览/编辑模式 | `⌘/` | `Ctrl+/` |
| 保存编辑中的文件 | `⌘S` | `Ctrl+S` |
| Skill 浏览器内聚焦搜索框 | `/` | `/` |

> Esc 仅在 Muxvo UI 层有焦点时生效，终端内 Esc 直接透传给 PTY。

**快捷键冲突检测说明：**
- `Cmd+W`（macOS）/ `Ctrl+W`（Win/Linux）需在 Electron 层拦截默认关闭窗口行为，改为关闭当前终端。通过 `BrowserWindow` 的 `close` 事件拦截或覆盖 Electron 默认菜单中的 Close Window 加速键实现。
- `Cmd+[` / `Cmd+]`（macOS）需 `preventDefault` 拦截浏览器默认的前进/后退导航行为，确保快捷键仅用于终端切换。
- 后续版本可考虑自定义快捷键能力（允许用户重映射快捷键绑定）。

**i18n 备注：**
- V2 Showcase 展示页需支持中英文切换，预留 react-intl 或 i18next 集成点
- V1 阶段暂不实施 i18n，但组件文案建议集中管理以降低后续迁移成本

**a11y 备注：**
- 后续版本规划 ARIA 标签和键盘导航支持
- V1 优先确保基础键盘可操作性（Tab 焦点管理、Esc 优先级），完整 a11y 合规留待后续迭代
...
| 项目 | 说明 |
|------|------|
| 职责 | PTY 进程创建/销毁/通信、终端状态管理（6.2 状态机）、前台进程检测（tcgetpgrp）、xterm.js 渲染、Grid 布局计算、拖拽排序、边框调整、同目录归组、cwd 切换（6.7 状态机）、内存监控（每 60s 检查 `process.memoryUsage()`，超 2GB 菜单栏警告） |
...
| 项目 | 说明 |
|------|------|
| 职责 | 富文本编辑器渲染（contenteditable / CodeMirror 6）、图片粘贴处理、多行协议转换（CC 用 \x1b\r、bash 直接发送）、Alternate Screen Buffer 检测（编辑器/原始终端模式切换，6.13 状态机）、Ctrl+C/Z/D 穿透、临时文件清理 |
| 对外接口 | `RichEditorComponent`, `getEditorContent(id)`, `clearEditor(id)`, `setEditorMode(id, 'rich' \| 'raw')` |
| 依赖模块 | terminal（PTY 写入、前台进程类型判断） |
| 临时文件清理策略 | ① 24h 定时清理：Main Process 定时扫描 `/tmp/muxvo-images/`，删除超过 24 小时的文件；② 终端关闭时清理：终端 close 事件触发时删除该终端关联的临时图片文件；③ 文件锁（`.lock`）防止读写冲突：写入时创建 `.lock` 文件，读取时检查锁，清理时跳过有锁的文件 |

### 4.3 聊天历史模块 (chat-history)

| 项目 | 说明 |
|------|------|
| 职责 | 读取 CC history.jsonl 和 session JSONL、增量镜像同步到 Muxvo 数据目录、全文搜索索引构建与查询、会话列表与详情展示（6.10 状态机）、聊天历史导出（对应任务 X3，支持 Markdown/JSON 格式导出） |
...
| Markdown 暗色主题配色（PRD 8.2.1） | 预览区背景：透明（跟随中栏背景）、正文颜色 `#e5e5e5`、标题/粗体颜色 `#ffffff`、链接颜色 `#6bb3f8`、代码块背景 `#2d2d2d`、行内代码颜色 `#f8a5c2`、引用块边框 `#444`、引用块文字 `#999`、分割线/表格边框 `#333`、表头背景与代码块背景一致 |
| Markdown 排版参数（PRD 8.2.1） | 基础字号 15px、行高 1.8、正文字体 `-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif`、代码字体 `SF Mono, Monaco, Courier New, monospace`、内容区内边距 24px（上下）× 32px（左右） |

### 4.5 配置管理模块 (config-manager)

| 项目 | 说明 |
|------|------|
| 职责 | CC 资源浏览（Skills/Hooks/Plans/Tasks/Settings/CLAUDE.md/Memory/MCP）、Plugin 资源浏览与管理、Settings 和 CLAUDE.md 编辑保存、配置管理器面板（6.11 状态机） |
...
| 项目 | 说明 |
|------|------|
| 职责 | 多源聚合浏览（6 个来源）、搜索与过滤、包详情展示、安装/卸载触发（6.14 + 6.15 状态机）、Hook 安全审查对话框、`/` 快捷键聚焦搜索框 |
...
| UI 规格 | 左侧边栏宽度 200px（来源筛选 + 类型筛选），右侧主区域为搜索框 + 包卡片列表（PRD §8.5） |
...
| 外部仓库 | muxvo-skill-scorer 为独立 CC Skill 仓库（github.com/muxvo/muxvo-skill-scorer），Muxvo 内置安装包并管理自动安装（首次评分时检测是否已安装，未安装则自动触发安装） |
...
| 外部仓库 | muxvo-publisher 为独立 CC Plugin 仓库（github.com/muxvo/muxvo-publisher），通过 P2-8 任务开发 |
| DOMPurify 清洗规则 | 允许标签白名单：`h1-h6`/`p`/`a`/`img`/`code`/`pre`/`ul`/`ol`/`li`/`blockquote`/`table`/`thead`/`tbody`/`tr`/`th`/`td`/`svg`；图片域名限制：仅允许 `github.com`、`githubusercontent.com`；移除所有 `on*` 事件属性和 `javascript:` 协议链接 |
...
| Onboarding（首次引导） | 首次使用引导流程组件归属于 layout 模块，包含：欢迎页、终端创建引导、功能区域高亮提示。组件位于 `src/renderer/components/onboarding/` |
...
| 依赖模块 | 无（被各功能模块调用） |
| 事件分类 | **V1（22 个）**：终端事件 6 个（create/close/switch/resize/rename/cwd-change）、面板事件 8 个（file-panel-open/close、chat-panel-open/close、config-panel-open/close、skill-browser-open/close）、富编辑器事件 5 个（activate/deactivate/image-paste/multiline-submit/mode-switch）、生命周期事件 3 个（app-launch/app-quit/session-duration）。**V2（16 个）**：评分事件 4 个（score-start/score-complete/score-cached/score-fail）、展示页事件 5 个（showcase-generate/publish/unpublish/share/view）、认证事件 3 个（auth-login/logout/token-refresh）、市场事件 4 个（install/uninstall/search/update） |
| 存储位置 | `~/Library/Application Support/Muxvo/analytics.json`（macOS），其他平台对应 Electron `app.getPath('userData')` |
| V1 scope 说明 | V1 仅采集本地事件，不上传至任何服务器；数据用于使用统计面板 UI 展示和可选纳入 AI 评分报告 |

> **Review 数据结构说明**：Review（用户评价/评论）相关数据结构归属 V2-P3 社区平台（showcase.muxvo.com），不在 V1/V2-P1/V2-P2 范围内实现。V2-P3 将定义 Review 的 API 接口、数据模型和展示组件。

### 4.13 社区模块预留说明 (community) — V2-P3

> V2-P3 将新增 community 模块，作为 Showcase 社区平台（showcase.muxvo.com）的后端项目，不在当前模块拆分范围内。此处仅做预留说明：community 模块将包含用户评价（Review）、社区排行榜、评论系统等功能，以独立后端服务形式开发，与 Muxvo 桌面端通过网络 API 交互。
...
│   │   │   └── memory-monitor.ts  # 内存监控（每 60s 检查，超 2GB 警告）
│   │   └── utils/
...
│   │   │   ├── analytics.store.ts
│   │   │   └── publish-draft.store.ts  # V2-P2 发布草稿管理
│   │   │
...
│   │   │   │   ├── FocusedView.tsx
│   │   │   │   └── CwdSwitcher.tsx
│   │   │   ├── rich-editor/         # 富编辑器独立目录（从 terminal/ 移出）
│   │   │   │   ├── RichEditor.tsx
│   │   │   │   ├── EditorToolbar.tsx
│   │   │   │   └── SlashCommand.tsx
│   │   │   ├── file-browser/
...
│       │   ├── analytics.types.ts
│       │   └── publish-draft.types.ts  # V2-P2 发布草稿类型定义
│       └── constants/
...
**数据目录说明：**
- analytics.json 存储路径：`~/Library/Application Support/Muxvo/analytics.json`（macOS），Windows 为 `%APPDATA%/Muxvo/analytics.json`，Linux 为 `~/.config/Muxvo/analytics.json`（均通过 Electron `app.getPath('userData')` 获取）
...
**V1 合计：16-19 周（1 个全栈开发者）**

---

## 9. V1 功能任务拆分

### 功能 A — 全屏平铺终端管理（V1-P0，10.5 天）

| 任务 | 涉及层 | 前置依赖 | 工期 |
|------|--------|----------|------|
| A1: Electron 应用骨架搭建（主窗口、菜单栏 36px、底部控制栏） | Main + Renderer | 无 | 2天 |
| A2: node-pty 集成 — spawn shell 进程、stdin/stdout 通信（含 electron-rebuild 编译验证） | Main | A1 | 2天 |
...
| A5: 终端进程生命周期管理（状态机 + 状态点颜色/动画映射 + 关闭确认对话框 + 含最大终端数 20 上限检测和提示） | Main + Renderer | A2 | 1.5天 |
| A6: config.json 持久化（终端列表、Grid 布局保存/恢复） | Main + 数据层 | A4 | 0.5天 |
| A7: WaitingInput 检测与 UI 处理（检测终端输出中选项列表模式 + 编辑器交互逻辑） | Main + Renderer | A5 | 0.5天 |
...
| B3: 3D 视觉效果（hover 倾斜 ±4°、光泽层、聚焦动画、状态点呼吸脉冲、背景渐变光斑、入场动画）。运动参数：hover transform 200ms ease-out、光泽层 linear-gradient 45deg、状态点脉冲 2s infinite、tile 入场 stagger 50ms delay | Renderer | B1 | 1.5天 |
...
### 功能 D — 聊天历史浏览器（V1-P0，8.5 天）

| 任务 | 涉及层 | 前置依赖 | 工期 |
|------|--------|----------|------|
| D1: JSONL 解析器（流式逐行读取、格式错误跳过、不完整末尾行忽略、CC 格式兼容性处理，含格式版本检测 + 未知字段忽略） | 数据层 | 无 | 1天 |
...
| D6: 中栏会话卡片（标题+时间+预览+标签，按时间倒序）。自动分类标签：feat/fix/refactor/plan 关键词匹配 + 工具调用计数显示 | Renderer | D4 | 0.5天 |
| D7: 右栏会话详情（气泡式消息渲染、Markdown 渲染、工具调用折叠、代码块高亮）。渲染规格：用户消息右对齐 accent 背景、助手消息左对齐 elevated 背景、工具调用蓝色左边框可折叠、工具结果绿色左边框。分页加载每页 50 条 + 滚动触底加载 | Renderer | D4 | 1.5天 |
| D8: 三栏联动交互（含聊天历史排序选项：按时间/按项目） | Renderer | D5, D6, D7 | 0.5天 |
| D9: 虚拟滚动（会话列表 50 条阈值触发虚拟化 + 详情分页每次 50 条） | Renderer | D4 | 0.5天 |
...
| RE2-2: 进程名检测 fallback（含手动模式切换快捷键 Cmd+Shift+E） | Main | I3, RE2-1 | 1天 |
...
| X2: 快捷键系统（含 Electron 层快捷键冲突检测与拦截：Cmd+W 关闭终端非窗口、Cmd+[/] 拦截浏览器导航） | Renderer | A4 | 1天 |
...
**X5 说明**：建立统一的异常处理和错误提示框架，覆盖 PRD 11.1 定义的 32 条异常场景。包括：错误提示组件（Toast/Dialog）、重试机制（自动重试 + 手动重试按钮）、降级策略（功能不可用时显示友好提示而非崩溃）、内存监控（每 60 秒检测，超 2GB 黄色警告图标）。各功能模块关键异常处理需求：
- **A（终端管理）**：PTY spawn 失败重试（最多 3 次，间隔 3 秒，最终失败显示错误面板含具体错误信息）、shell 异常退出恢复、前台进程检测失败降级
- **D（聊天历史）**：JSONL 解析失败跳过、同步中断恢复、CC 目录不存在降级
- **H（文件浏览）**：目录权限不足降级提示、chokidar 监听失败重试、磁盘空间不足提示清理
- **RE（富编辑器）**：图片粘贴失败 fallback 文本路径、临时文件清理异常静默处理
- **E（全文搜索）**：索引构建超时保护、搜索无结果友好提示
- **O（一键安装）**：下载失败自动重试 1 次、SHA256 完整性校验 + 安装路径权限检查、版本冲突提示用户选择
- **SR（AI 评分）**：评分结果 JSON 解析失败重试 1 次、评分超时保护
- **SS（展示页）**：图片上传失败自动压缩重试、GitHub 仓库删除检测标记 Unpublished、名称冲突提示改名
- **P2（发布分享）**：Pages 发布超时 30s 自动保存草稿、Publisher 未授权提示登录、微信分享图生成失败回退复制链接

**X5 异常场景→负责任务完整映射表**（PRD 11.1 共 32 条）：

| # | 异常场景 | 负责任务 | 处理方式 |
|---|---------|---------|---------|
| 1 | 前台进程崩溃 | A5 | 状态变红 + 重启按钮 |
| 2 | 文件读取失败 | H1 | 错误提示 + 权限检查 |
| 3 | JSONL 解析错误 | D1 | 跳过错误行，静默处理 |
| 4 | ~/.claude/ 目录不存在 | D1, X1 | 功能降级，引导安装 CC |
| 5 | 磁盘空间不足 | X5 | Toast 提示清理 |
| 6 | 网络不可用 | N2-8 | 离线提示，展示本地数据 |
| 7 | 下载失败 | O-1 | 自动重试 1 次，仍失败提示 |
| 8 | 包完整性校验失败 | O-2 | 拒绝安装，SHA256 校验 + 提示重新下载 |
| 9 | 安装路径无写入权限 | O-2 | 安装路径权限检查 + 提示 |
| 10 | 发布时未登录 | P2-5 | 引导 GitHub 登录 |
| 11 | GitHub OAuth 失败 | P2-5 | 提示重试 |
| 12 | CC 终端未运行（AI 评分） | SR-3 | 提示启动 CC |
| 13 | 评分 Skill 未安装 | SR-3 | 自动安装 muxvo-skill-scorer |
| 14 | 评分 Skill 运行失败 | SR-5 | 最多重试 3 次 |
| 15 | 评分结果 JSON 解析失败 | SR-5 | 重新评分 1 次 |
| 16 | 聚合源返回错误 | N2-8 | 切换到其他可用源 |
| 17 | 已安装包版本冲突 | O-4 | 提示用户选择覆盖/保留 |
| 18 | CC 聊天记录被清理 | D2 | 自动切换到 Muxvo 镜像 |
| 19 | Muxvo 镜像目录权限不足 | D3 | 降级为纯只读模式 |
| 20 | 同步过程中 CC 文件被锁定 | D3 | 跳过该文件，下次重试 |
| 21 | CC 数据格式变更 | D1 | 版本检测 + 降级处理 |
| 22 | SKILL.md 解析失败 | SS-2 | 切换手动填写模式 |
| 23 | GitHub Pages 发布超时 | P2-7 | 30s 超时保存草稿 |
| 24 | GitHub API rate limit | P2-7 | 提示配额重置时间 |
| 25 | 展示页图片上传失败 | SS-3 | 自动压缩重试（单张 ≤ 5MB） |
| 26 | GitHub 仓库被手动删除 | SS-6 | 检测不存在→标记 Unpublished |
| 27 | 发布安全检查不通过 | P2-4 | 阻止发布，标红具体行号 |
| 28 | 名称已被占用 | P2-7 | 提示冲突，建议改名 |
| 29 | 网络中断（发布中） | P2-7 | 保存草稿到本地 |
| 30 | 微信分享图生成失败 | P2-9 | 回退为复制链接 |
| 31 | Deep Link 未响应 | O-4 | 3s 后展开手动安装选项 |
| 32 | 对话式发布 GitHub 未授权 | P2-8 | 提示在 Muxvo GUI 中登录 |

**X4 缺省态场景清单**（约 20 个场景，对应 PRD 11.3）：

- **终端区域**：无终端→引导创建（"按 ⌘T 新建终端" + [新建终端]）；全部关闭→提示新建（[新建终端] [恢复上次布局]）
- **聊天历史**：未安装 CC→引导安装（[了解 Claude Code]）；无聊天记录→提示使用 CC；搜索无结果→[清除搜索]；索引构建中→进度条
- **文件浏览**：空目录→文件夹图标；加载中→旋转图标；无权限→锁图标
- **配置管理**：无 Plans/Skills/Hooks→各提示创建；Settings 读取失败→警告图标 + [重试]
- **Skill 浏览器（V2）**：首次加载→[浏览 Skills]；搜索无结果→[清除搜索]；无已安装→[浏览 Skills]；源不可用→[重试]
- **Showcase（V2）**：无 Skill→提示创建；解析失败→[手动填写]；未评分→[立即评分]；未登录→[登录 GitHub]/[导出 HTML]；发布失败→[重试]/[导出 HTML]；CDN 未生效→[刷新检查]

**X7/X8 数据埋点事件分类**：

V1 事件（共 22 个，与 PRD §13.3 对齐）：
- 终端管理 6 个：`terminal.create`（参数 `{cwd}`） / `terminal.close`（参数 `{cwd, duration_seconds, had_foreground_process}`） / `terminal.focus`（参数 `{terminal_index}`） / `terminal.cwd_switch`（参数 `{from_cwd, to_cwd, method: "direct_cd" | "exit_and_cd"}`） / `terminal.drag_reorder`（参数 `{terminal_count}`） / `terminal.resize_border`
- 面板 8 个：`panel.file_open`（参数 `{cwd}`） / `panel.file_preview`（参数 `{file_type}`） / `panel.history_open` / `panel.history_search`（参数 `{query_length, result_count}`） / `panel.history_export`（参数 `{format, session_count}`） / `panel.config_open` / `panel.config_category`（参数 `{category}`） / `panel.skill_browser_open`
- 富编辑器 5 个：`editor.activate`（参数 `{terminal_index}`） / `editor.image_paste`（参数 `{image_size_kb, source: "clipboard"|"drag"}`） / `editor.multiline_send`（参数 `{line_count, char_count}`） / `editor.fallback_to_terminal`（参数 `{trigger_program}`） / `editor.return_from_terminal`（参数 `{fallback_duration_seconds}`）
- 生命周期 3 个：`app.launch`（参数 `{terminal_count_restored, version, platform}`） / `app.quit`（参数 `{session_duration_minutes, terminal_count}`） / `app.onboarding_complete`（参数 `{skipped: boolean}`）

V2 事件（共 16 个）：
- Skill 发现 4 个：`skill.search` / `skill.detail_view` / `skill.install` / `skill.uninstall`
- AI 评分 5 个：`score.generate` / `score.result` / `score.fail` / `score.save_image` / `score.copy_image`
- Showcase 7 个：`showcase.generate` / `showcase.template_switch` / `showcase.screenshot_add` / `showcase.publish` / `showcase.publish_fail` / `showcase.share_link_copy` / `showcase.update`
...
```json
{
  "version": 1,
  "events": [
    { "event": "terminal.create", "timestamp": "ISO8601", "params": { "cwd": "/path" } }
  ],
  "daily_summary": {
    "2026-02-08": {
      "date": "2026-02-08",
...
**数据保留与聚合策略**：
- 每日 00:00 聚合前日明细为 `daily_summary` 摘要
- 明细事件（`events` 数组）：保留 90 天，超过自动清理
- 每日摘要（`daily_summary`）：保留 1 年，超过自动清理
- 清理时机：每次 `app.launch` 时检查并执行
...
- 图片懒加载：Skill 浏览器中用户头像使用 IntersectionObserver 延迟加载（已描述方案，具体 UI 实现将在 V2 N2-9 任务中落地）
...
- 模式切换：ASB 信号 → 进程名检测（RE2-2）→ 手动快捷键兜底（Cmd+Shift+E，对应 RE2-2 任务）
...
- 仅同步不删除，文件锁定时跳过下次重试
- 批量文件变化时 debounce 500ms 合并同步操作，单次同步最多处理 50 个文件，避免频繁 IO
...
- 更新检测：启动时 + 每 24 小时检查。所有已安装包的更新检查批量合并为一次 API 调用，减少请求数
...
**方案**：
- JSON 结构化日志格式：`{ timestamp, level, module, message, stack? }`
- 三级日志：`error`（异常/崩溃）、`warn`（降级/重试）、`info`（关键操作）
- 存储位置：`~/Library/Application Support/Muxvo/logs/`
- 轮转策略：单文件 10MB 自动轮转、保留最近 30 天
- 实现方案：集成 `electron-log` 或自建轻量 logger（文件写入 + 日志级别过滤）

### 11.12 崩溃恢复策略

**方案**：
- config.json 写入使用原子操作：先写入 `.config.json.tmp` 临时文件，成功后 `fs.rename()` 覆盖原文件，避免写入中断导致文件损坏
- 启动时检测 `.crash-flag` 文件判断上次是否异常退出：正常启动时创建 `.crash-flag`，正常退出时删除；若启动时 `.crash-flag` 已存在，说明上次异常退出
- 异常退出恢复：读取上次 config.json 中的终端列表，尝试在相同目录重建终端
- PTY 进程不恢复（已终止），仅恢复目录和布局配置
...
| analytics 模块 | 事件采集、每日聚合、90 天明细清理、1 年摘要清理 | ~10 |
| config 持久化 | config.json 读写、原子写入（.tmp → rename）、文件损坏恢复 | ~8 |
| cwd 切换逻辑 | shell 直接 cd、AI 工具退出后 cd、QuickAccess 选择、进程检测分支 | ~6 |
| 自动归组算法 | 同目录检测、归组触发、cwd 变更后拆组、边界情况 | ~5 |
| WaitingInput 检测 | 选项列表模式匹配、CC 提示符检测、误检排除 | ~5 |
...
| cwd 切换端到端 | 点击 cwd → QuickAccess 弹窗 → 选择目录 → 进程检测 → cd 执行 → 名称更新 |
| 自动归组端到端 | 新建终端 → 检测同目录 → 归组动画 → cwd 切换 → 拆组验证 |
| 快捷键系统端到端 | 快捷键触发 → Esc 优先级验证 → 视图模式切换 → 面板开关 |
| 数据埋点端到端 | 事件触发 → 采集写入 → analytics.json 存储 → 每日聚合 → 90 天清理 |
| 聊天导出端到端 | 选择会话范围 → 选择导出格式 → 文件写入 → 验证内容完整 |
...
| 引导流程 | 启动（首次） → 检测首次使用 → 引导步骤逐步完成 → 创建首个终端 → 完成引导 |
| 拖拽排序+边框调整 | 拖拽终端卡片 → 验证布局位置变化 → 拖拽边框 → 验证列比例更新 → 双击重置等分 |
| cwd 切换旅程 | 点击 cwd 区域 → QuickAccess 浏览器弹出 → 选择新目录 → 验证 Header 名称更新 → 验证终端 cd |
| 同目录归组 | 新建 2 个同目录终端 → 验证自动归组效果 → 切换其中一个 cwd → 验证拆组 |
| 时间线视图切换 | 切换到时间线视图 → 验证按天分组显示 → 选择会话 → 查看详情 → 切换回三栏视图 |
...
| 更新检测引擎 | 语义版本比较、注册表遍历、定时触发逻辑 | ~8 |
| Hook 安全审查 | Hook 源码解析、风险关键词匹配（curl/eval/rm -rf）、高亮标注 | ~6 |
| SKILL.md 解析器 | YAML frontmatter 解析、fallback 手动填写、格式异常处理 | ~5 |
| Deep Link 协议 | URL 格式解析（muxvo://install/...）、协议注册验证 | ~5 |
...
| Hook 安全审查端到端 | 安装含 Hook 的 Skill → 弹出审查对话框 → 查看源码 → 风险高亮 → 确认/取消 |
| 更新检测端到端 | 启动/定时触发 → 版本比较 → 徽章显示 → 点击更新 → 安装验证 |
...
| 更新流程 | 发现可更新包 → 验证徽章显示 → 点击更新 → 下载安装 → 验证版本更新 |
| 重评分循环 | 修改 SKILL.md → 检测内容变化 → 重新触发评分 → 验证分数/等级变化 |
| 外部浏览者旅程 | 打开展示页 URL → 查看展示页内容 → 点击 Deep Link 安装按钮 → 验证 muxvo:// 协议触发 |
...
- 字体渲染差异
- 系统通知与托盘图标
- CC/Muxvo 配置目录路径映射（macOS `~/.claude/` vs Windows `%APPDATA%/claude/`）
- Token 安全存储差异（macOS Keychain / Windows DPAPI / Linux libsecret）
- Deep Link 协议注册验证（各平台 muxvo:// 协议注册与响应）
- tcgetpgrp 跨平台替代方案（macOS/Linux 原生 vs Windows 备选方案）
...
**交付物**：时间线视图 + 分类查看 + 编辑功能 + 富编辑器完善 + X5 异常处理框架 + X6 自动更新与签名 + X7 数据埋点框架（22 个 V1 事件）+ X8 使用统计面板
...
| W5 | Bug 修复缓冲 + 性能调优 | 稳定版本 |

**验收标准**：
- 聚合浏览器可展示 3+ 源数据，搜索返回结果 < 500ms
- 安装/卸载流程完整（下载 → 解压 → 注册 → 列表刷新 → 卸载 → 清理）
- Hook 审查弹窗功能正常（源码展示 + 风险关键词高亮 + 确认/取消）
...
**验收标准**：
- 评分一致性偏差 < 5 分（同 Skill 多次评分）
- 六维度雷达图正确渲染，评分卡 PNG 导出功能正常
- 更新检测徽章正确显示（红色数字徽章 + amber 标记）
...
**验收标准**：
- 展示页生成 < 3 秒，GitHub Pages 发布成功率 > 95%
- 分享链接可正常访问，OG Card 在社交平台正确展示
- Deep Link 安装正常（muxvo:// 协议触发 → 应用启动 → 安装流程）
...
| N2-9 | 浏览器全屏覆盖层 UI。左侧边栏宽度 200px（PRD §8.5），含来源筛选 + 类型筛选 + 排序选项（来源优先/名称字母序）。埋点：`skill:browser-open` | UI/Renderer | N2-8 | 2d |
...
| O-2 | 包解压 + 文件放置。含 SHA256 完整性校验：下载完成后校验包 hash，不匹配则拒绝安装并提示重新下载 | Main | O-1 | 0.5d |
...
| # | 任务 | 涉及层 | 前置依赖 | 预估 |
|---|------|--------|---------|------|
| SR-1 | muxvo-skill-scorer Skill 开发（Prompt + 6 维度 + JSON）。独立 GitHub 仓库 `muxvo/muxvo-skill-scorer`，Muxvo 首次评分时自动安装。埋点：`score:scorer-install` | CC Skill | 无 | 2d |
| SR-2 | Prompt Injection 防护 | CC Skill | SR-1 | 0.5d |
| SR-3 | 评分触发机制（检测 CC 终端 → 发送指令）。含评分 Skill 自动安装检测：首次评分 → 检查 scorer 是否安装 → 未安装触发自动安装 → 安装完成自动触发评分。含 `includeAnalytics` 授权弹窗：首次勾选时弹出确认对话框，说明将纳入哪些使用数据（终端使用时长、面板操作频次等），用户确认后设置偏好。埋点：`score:trigger` / `score:complete` | Main | V1 终端管理, SR-1 | 1d |
...
| SS-4 | 展示页编辑/预览 UI。埋点：`showcase:screenshot-add` / `showcase:update`（参数 `{skill_name, changes}`） | UI/Renderer | SS-3 | 2d |
...
| P2-5 | GitHub Pages 自动发布。异常处理：GitHub Pages 发布超时 → 自动保存为草稿。埋点：`showcase:publish` / `showcase:publish-success` / `showcase:publish-fail`（参数 `{skill_name, error_type}`） | Service | P2-1, SS-3 | 2d |
...
- 图片 `src` 域名白名单：`github.com`, `raw.githubusercontent.com`, `user-images.githubusercontent.com`, `i.imgur.com`
...
正则扫描引擎逐行匹配：API Key (`sk-*`, `ghp_*`, `AKIA*`, `xoxb-*`)、密钥文件 (`-----BEGIN PRIVATE KEY-----`)、硬编码路径 (`/Users/*/`, `C:\\Users\\`)、敏感文件 (`.env`, `.pem`)、文件大小限制（单个 Skill 超过 1MB 或 Plugin 超过 10MB 时显示警告，PRD §8.8）。返回 `{ passed, issues: [{ type, line, column, snippet, severity }] }`。
...
| 扩展点 | 位置 | 说明 |
|--------|------|------|
| IPC 通道命名空间 | Main | 预留 `marketplace:*`、`score:*`、`showcase:*`。包名统一使用 `@scope/name` namespace 格式 |
...
'score:result'                  // 评分结果（score 域，已在 §2.7 补充定义）
'showcase:publish-result'       // 发布结果（showcase 域，已在 §2.8 补充定义）
'marketplace:update-available'  // 更新可用通知（marketplace 域）

// Renderer → Main
'marketplace:fetch-sources'     // 加载聚合数据（marketplace 域，与 §2.6 一致）
'marketplace:install'           // 安装包（marketplace 域，与 §2.6 一致）
'marketplace:uninstall'         // 卸载包（marketplace 域，与 §2.6 一致）
...
// 终端状态查询使用 terminal:get-state（已在 §2.1 补充定义）
```
...
| **V1-P1** | 6-7 周 | 拖拽排序 + 搜索 + 文件浏览 + 配置浏览 + 富编辑器基础 | 搜索索引性能、富编辑器↔PTY |
| **V1-P2** | 6-7 周 | 时间线 + 分类查看 + 编辑 + 富编辑器完善 + 埋点 + 异常处理 | ASB 检测、多工具协议 |
| **V1 合计** | **18-21 周** | | |
| **V2-P0** | 4-5 周 | 聚合浏览器 + 安装 + 审查 | 聚合源 API 可用性 |
...
> **V1 工期调整说明**：相比初始估算（15-18 周），V1 合计上调至 18-21 周。新增任务包括：A7 WaitingInput 检测（+0.5 天）、D9 虚拟滚动（+0.5 天）、X4 缺省态扩展至 20 个场景（+1.5 天）、X5 异常处理框架含 32 条映射（+1 天）、X6 Electron 自动更新与签名（+2 天）、X7 数据埋点框架（+3 天）、X8 使用统计面板（+2 天），合计新增约 10.5 天。V1-P1 从 5-6 周上调至 6-7 周（29d 功能任务在 25d 下限时溢出），V1-P2 从 4-5 周上调至 6-7 周。V2-P0 从 3-4 周上调至 4-5 周（18.5d 功能任务在 15d 下限时溢出）。
