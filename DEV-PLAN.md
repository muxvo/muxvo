...
| D7: 右栏会话详情（气泡式消息渲染、Markdown 渲染、工具调用折叠、代码块高亮）。渲染规格：用户消息右对齐 accent 背景、助手消息左对齐 elevated 背景、工具调用蓝色左边框可折叠、工具结果绿色左边框。分页加载每页 20 条 + 滚动触底加载 | Renderer | D4 | 1.5天 |
...
- **A（终端管理）**：PTY spawn 失败重试、shell 异常退出恢复、前台进程检测失败降级
...
V1 事件（共 22 个）：
- 终端管理 6 个：`terminal.create` / `terminal.close` / `terminal.focus` / `terminal.layout_change` / `terminal.rename` / `terminal.process_detect`
- 面板 8 个：`panel.file_open` / `panel.file_close` / `panel.history_open` / `panel.history_close` / `panel.config_open` / `panel.config_close` / `panel.skill_browser_open` / `panel.skill_browser_close`
- 富编辑器 5 个：`editor.activate` / `editor.send` / `editor.paste_image` / `editor.mode_switch` / `editor.abort`
- 生命周期 3 个：`app.launch` / `app.quit` / `app.window_resize`
...
```json
{
  "version": 1,
  "events": [
    { "name": "terminal.create", "params": { "cwd": "/path" }, "timestamp": "ISO8601" }
  ],
  "daily_summary": {
    "2026-02-08": {
      "date": "2026-02-08",
