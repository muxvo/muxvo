...
- AI 评分通过内置 CC Skill 在本地完成，SKILL.md 内容不经过 Muxvo 服务器
...
- V2 前期不自建 Web 商城，通过内置聚合浏览器 + GitHub Pages 展示页覆盖浏览和发布需求，聚合已有生态（Anthropic 官方仓库、SkillsMP、GitHub awesome-lists 等）
...
| SR | AI Skill 评分 | 通过内置 CC Skill（muxvo-skill-scorer）对 SKILL.md 进行多维度评分（实用性/工程质量/意图清晰度/设计巧妙度/文档完善度/可复用性），生成雷达图评分卡 + 等级称号，支持保存为 PNG/复制分享 | V2-P1 | 痛点7 |
| RE3 | 富编辑器覆盖层（高级） | 斜杠命令自动补全（CC /clear /compact 等）、输入历史（上箭头召回）、文件拖拽、代码块语法高亮 | V2-P2 | 痛点10 |
...
├── marketplace.json                   # 本地注册表（安装记录 + 使用统计）
├── skill-scores/                      # AI 评分结果缓存（每个 Skill 一个 JSON）
├── showcases/                         # Showcase 展示页配置（每个 Skill 一个 JSON）
├── analytics.json                     # 本地埋点数据
...
**Skills 安装（来源：聚合源）：**
- 主路径：直接文件操作——下载 → 校验 → 解压到 `~/.claude/skills/{name}/` → 完成
- 可选增强：若 CC 终端运行中且空闲，向 CC 发送 `/plugin install {name}` 以触发 CC 原生的安装后处理（如依赖检查）
- chokidar 监听目录变化，Skills 列表自动刷新，无需手动操作
- 注：CC 命令安装路径依赖 CC 支持自定义 marketplace 源，如不支持则统一使用文件操作安装
...
```
发布：作者本地 Skill → 安全检查 → AI 评分 → 填写详情 → 发布到商城
分享：商城 → 一键分享到 Twitter/LinkedIn/微信/Discord/Reddit/README 徽章
安装：社交平台/商城 → 一键安装到本地
循环：用得好 → 也来发布 → 飞轮转动
```
...
通过内置 CC Skill（`muxvo-skill-scorer`）对 SKILL.md 进行多维度评分。评分 Skill 在 CC 内运行，利用 CC 已有的 API 访问权限，用户无需额外配置 API Key：

| 维度 | 权重 | 说明 |
|------|------|------|
| 实用性 | 25% | 解决真实问题的程度，使用场景是否清晰 |
| 工程质量 | 25% | Prompt 结构、错误处理、边界条件覆盖 |
| 意图清晰度 | 10% | Skill 目标是否明确、解决的问题是否定义清晰 |
| 设计巧妙度 | 10% | Prompt 设计、工具使用、工作流编排是否巧妙 |
...
发布成功后弹出分享面板，支持多渠道一键分享（详见 8.8 发布到商城、8.10 展示页）。
...
Muxvo 在本地记录包安装信息和使用分析数据，详细数据结构定义见 7.5 节。

使用分析增强：
- 追踪每个已安装 Skill 的调用次数、使用会话数、最后使用时间、平均使用时长
- 数据仅存储在本地，用户可在 Skills 列表中查看自己的使用统计
- 可选：在 AI 评分时纳入使用数据作为"实用性"评分的辅助参考（需用户授权）

### 3.11 商城形态策略

> **命名说明**：本文档中的「商城」是对 Muxvo Skill 展示与分发体系的统称，涵盖内置聚合浏览器（V2-P0）、GitHub Pages 展示页（V2-P2）、showcase.muxvo.com 社区平台（V2-P3）三种形态。V2-P0/P1 阶段「商城」= Muxvo 内置聚合浏览器，不涉及自建 Web 商城。

Muxvo 商城分三个阶段演进：

| 阶段 | 形式 | 说明 |
|------|------|------|
| V2-P0 | Muxvo 内置聚合浏览器 | App 内浏览多源 Skill/Plugin，本地安装 |
| V2-P2 | 展示页（GitHub Pages） | 每个已发布 Skill 的独立 Web 展示页，可社交分享 |
| V2-P3 | showcase.muxvo.com | 完整 Web 商城：Feed 流、搜索、排行榜、个人主页 |

V2-P2 阶段分享链接指向 GitHub Pages 展示页。V2-P3 上线后统一指向 showcase.muxvo.com。V2-P0/P1 阶段不支持外部分享。

**聚合浏览器数据源：**

聚合 6 个来源：Muxvo 商城、Anthropic 官方、42plugin、GitHub 搜索、SkillsMP、本地已安装。详细数据获取方式和标识见 8.5 节。

**V2-P2 展示页索引方案：**
- 用户通过 Muxvo 发布时，Muxvo 同步在 `github.com/muxvo/showcase-index` 公开仓库提交一条索引记录（JSON）
- 索引记录包含：skill name、展示页 URL、评分等级、作者、发布时间
- Muxvo 内置浏览器的「Muxvo 商城」数据源即读取此索引仓库
- V2-P3 上线后，索引迁移到 showcase.muxvo.com 数据库
...
| 阶段 | 用户动作 | 系统反馈 | 关键目标 |
|------|----------|----------|----------|
| AI 评分 | 在 Skills 列表中选择本地 skill，点击「AI 评分」 | 触发 CC 运行评分 Skill → 显示雷达图 + 总分 + 等级称号 | 了解 Skill 质量 |
...
flowchart TD
    A[用户点击「安装」] --> B{包类型?}
    B -->|纯 Skill| G[直接文件操作：下载 → 校验 → 解压到 ~/.claude/skills/]
    B -->|含 Hook/代码| D[弹出安全审查对话框]
    D --> E{用户确认?}
    E -->|取消| F[取消安装]
    E -->|确认| G
    G --> I[安装完成]
    I --> J[写入本地注册表]
    J --> K["按钮变为「已安装 ✓」"]
    K --> L["chokidar 检测新文件，列表刷新"]
    L --> M["显示使用引导：在 CC 中说 {示例提示词}"]
    I -.->|可选：CC 运行中且空闲| N["向 CC 发送 /plugin install 触发安装后处理"]
```

### 5.6 Skill 发布与分享流程（V2-P2）

```mermaid
flowchart TD
    A[用户选中本地 Skill] --> B["点击「发布到商城」"]
    B --> C[安全检查：扫描密钥/敏感路径/敏感文件]
    C --> D{检查通过?}
    D -->|不通过| E[标红提示具体问题行号]
    D -->|通过| F{已有 AI 评分且内容未变?}
    F -->|否| G[自动触发 AI 评分]
    F -->|是| H[使用缓存评分]
    G -->|成功| H
    G -->|失败| H2["标记「暂无评分」，不阻止发布"]
    H --> I[填写详情：名称/描述/标签自动提取，用户可编辑]
    H2 --> I
    I --> J[预览商城页面]
    J --> K{已登录 GitHub?}
    K -->|否| L[GitHub OAuth 登录]
    L --> K
    K -->|是| M[发布到商城 + 生成展示页 + 推送到 GitHub Pages]
    M -->|成功| N[弹出分享面板]
    M -->|失败| O[保存草稿到本地 + 显示错误信息 + 重试按钮]
    N --> P{用户选择分享渠道}
    P -->|Twitter| Q[打开浏览器，预填文案+链接]
    P -->|LinkedIn| R[打开浏览器，预填专业文案]
    P -->|微信| S[生成分享图+QR码，保存到本地]
    P -->|复制链接| T[复制商城 URL 到剪贴板]
    P -->|复制徽章| U[复制 Markdown badge 代码]
    P -->|关闭| V[完成]
```

### 5.7 对话式发布流程（V2-P2）

> 用户在 CC 终端对话中直接发布，无需切换到 Muxvo GUI。

```mermaid
flowchart TD
    A["用户在 CC 中说「发布 xxx 到 Muxvo」\n或输入 /muxvo-publish skill-name"] --> B[muxvo-publisher Skill 触发]
    B --> C[安全检查]
    C --> D{通过?}
    D -->|不通过| E[终端提示具体问题]
    D -->|通过| F[调用 muxvo-skill-scorer 评分]
    F --> G[用自动提取的元数据作为详情]
    G --> H[通过 GitHub API 发布到商城 + Pages]
    H -->|成功| I["终端返回：商城链接 + 安装命令 + 分享文案"]
    I --> J[分享文案自动复制到剪贴板]
    J --> K["通知 Muxvo 生成分享图（如 Muxvo 运行中）"]
    H -->|失败| L[终端提示错误 + 重试建议]
```

与 GUI 发布的区别：跳过手动填写详情和预览步骤，适合快速发布。用户可随后在 Muxvo GUI 中补充截图、Problem/Solution 等详情。
...
flowchart TD
    ...
    CheckCache -->|否| CheckCC{CC 终端是否运行中?}
    CheckCC -->|否| ErrorCC[提示：请先启动一个 CC 终端]
    CheckCC -->|是| CheckSkill{评分 Skill 已安装?}
    CheckSkill -->|否| InstallSkill[自动安装 muxvo-skill-scorer Skill]
    CheckSkill -->|是| RunSkill[触发 CC 运行评分 Skill\n传入目标 SKILL.md 路径]
    InstallSkill --> RunSkill
    RunSkill --> Success{评分成功?}
    ...
    Error -->|重试| RunSkill
    Error -->|超过重试次数| ErrorFinal[显示最终错误 + 手动重试按钮]
```
...
stateDiagram-v2
    ...
    Authorizing --> LoggedIn: 收到 access_token
...
```json
{
  ...
  "scores": {
    ...
    "intentClarity": { "score": 90, "reason": "目标明确——根据 git diff 生成 commit message，问题定义清晰" },
    "designCleverness": { "score": 75, "reason": "工作流编排合理，但 prompt 设计较常规" },
...
### 7.9 muxvo-publisher Plugin 结构

> muxvo-publisher 是 Muxvo 配套的 CC Plugin，用于在 CC 终端中实现对话式发布。安装方式：Muxvo 首次检测到用户触发对话式发布时自动安装到 `~/.claude/plugins/muxvo-publisher/`。

```json
{
  "name": "muxvo-publisher",
  "type": "plugin",
  "components": {
    "skill": "SKILL.md",
    "command": "/muxvo-publish"
  },
  "config": {
    "tokenSource": "muxvo-keychain",
    "outputDir": "~/Library/Application Support/Muxvo/publish-drafts/"
  }
}
```

### 7.10 发布草稿（PublishDraft）

> 存储在 `~/Library/Application Support/Muxvo/publish-drafts/{skill-name}.json`，记录发布流程中的草稿状态、安全检查结果和发布详情。

```json
{
  "skillDirName": "commit-helper",
  "status": "draft|security_check_failed|ready|published",
  "securityCheck": {
    "passed": true,
    "issues": [],
    "checkedAt": "ISO8601"
  ...
  "details": {
    ...
    "problem": "用户手写",
    "solution": "用户手写",
    "screenshots": ["screenshots/demo1.png"]
  },
  "scoreRef": "commit-helper",
  "publishedUrl": "username.github.io/muxvo-skills/commit-helper",
  "publishedAt": "ISO8601",
  "version": "1.0.0"
}
```
...
| 来源 | 数据获取方式 | 标识 |
|------|-------------|------|
| Muxvo 商城 | Muxvo 用户发布的 Skill/Plugin | 🏪 Muxvo |
| Anthropic 官方 | CC 官方 Plugin Marketplace | 🏛 官方 |
| 42plugin | 第三方插件生态（需验证 API 可用性，如不可用则降级为网页数据抓取或移除） | 🌊 42plugin |
| GitHub 搜索 | 含 SKILL.md 或 plugin.json 的开源仓库 | 🐙 GitHub |
...
### 8.8 发布到商城（功能 P2，V2-P2）

**发布入口：**

| 入口 | 位置 | 场景 |
|------|------|------|
| Muxvo GUI | Skills 列表 → 选中本地 Skill → 点击「发布到商城」 | 主要入口，可精编详情 |
| CC 对话 | 在 CC 中说"发布 xxx 到 Muxvo" 或 `/muxvo-publish` | 快捷入口，适合快速发布 |

**发布前安全检查（自动）：**

| 检查项 | 说明 | 命中处理 |
|--------|------|---------|
| API Key / Token | 正则匹配常见密钥格式（sk-xxx、ghp_xxx 等） | 阻止发布，标红提示具体行号 |
| 硬编码路径 | 包含 `/Users/xxx/` 等本地绝对路径 | 警告，用户确认后可继续 |
| 敏感文件 | .env、credentials.json、私钥文件 | 阻止发布 |
| 文件大小 | 单个 Skill 超过 1MB / Plugin 超过 10MB | 警告，建议精简 |

**详情填写：**

| 字段 | 是否必填 | 自动提取 | 说明 |
|------|---------|---------|------|
| 名称 | 必填 | ✅ 从 SKILL.md 的 name 或目录名 | 商城展示名 |
| 一句话描述 | 必填 | ✅ 从 SKILL.md 的 description | 列表页摘要 |
| 分类标签 | 必填 | ✅ AI 自动推荐 | 开发工具/科研/内容创作/效率/其他 |
| 解决什么问题 | 选填 | ❌ | 用户手写，提升展示页质量 |
| 怎么解决的 | 选填 | ❌ | 用户手写 |
| 截图/GIF | 选填 | ❌ | 效果演示，最多 5 张 |
| 使用示例 | 选填 | ✅ 从 SKILL.md 提取 | 展示典型用法 |

- 自动提取的字段用户可修改
- 选填字段跳过不影响发布，但会影响展示页完整度
- 首次发布需要 GitHub OAuth 登录（后续免登录）

**版本与更新：**

| 场景 | 处理方式 |
|------|---------|
| 首次发布 | 创建商城页面，版本 1.0.0 |
| 更新内容 | 检测到本地 SKILL.md 变化 → Skills 列表显示「可更新到商城」→ 点击后上传新版本 |
| 重新评分 | 更新后自动重新评分，新旧评分都保留（展示分数变化趋势） |
| 下架 | 商城详情页 →「下架」→ 确认 → 从商城移除，已安装用户不受影响 |

**商城页面信息结构：**

见 8.10 展示页章节中的页面结构（Hero + 评分卡 + Problem/Solution + 截图 + 概况 + 作者信息 + 安装统计 + 品牌区）。

**Web 端安装引导（商城页面的安装按钮）：**

| 场景 | 行为 |
|------|------|
| 已安装 Muxvo | 触发 Deep Link `muxvo://install/skill-name` → Muxvo 打开并安装 |
| 未安装 Muxvo（3 秒无响应） | 展开安装选项卡：① 下载 Muxvo 一键安装（推荐）② CC 命令 `/plugin install skill-name@muxvo`（复制）③ 手动下载文件到 ~/.claude/skills/ |

**对话式发布（CC 终端内）：**

通过 `muxvo-publisher` CC Plugin（含 `/muxvo-publish` Command + Skill）实现。用户在 CC 中说"发布 xxx 到 Muxvo"或输入 `/muxvo-publish`，自动完成安全检查 → AI 评分 → 发布 → 返回商城链接和分享素材。与 GUI 发布的区别：跳过手动填写详情和预览步骤，后续可在 Muxvo GUI 补充。

**muxvo-publisher 安装方式：**
- 自动安装：用户首次在 Muxvo GUI 中触发「发布到商城」时，Muxvo 自动将 muxvo-publisher 安装到 `~/.claude/plugins/muxvo-publisher/`
- 手动安装：CC 终端中执行 `/plugin install muxvo-publisher@muxvo`
- 安装后用户在 CC 中可直接使用 `/muxvo-publish` 命令或自然语言触发发布

**Token 传递机制：**
- 对话式发布需要 GitHub Token（存储在 Muxvo Electron 的 safeStorage 中）
- 方案 A（推荐）：Muxvo 启动时将 Token 写入临时文件（权限 600），muxvo-publisher 读取后 Muxvo 立即删除
- 方案 B：通过环境变量注入到 Muxvo 启动的 CC 终端进程
- 具体方案待技术验证后确认（见附录 B 待确认项）

**分享图生成限制：**
- 分享图生成依赖 Muxvo Electron 的渲染能力，对话式发布时通过 IPC 通知 Muxvo 生成
- 如 Muxvo 未运行，跳过分享图生成，仅返回链接和文案

### 8.9 AI Skill 评分（功能 SR，V2-P1）

**功能概述**：通过内置 CC Skill（`muxvo-skill-scorer`）对本地 SKILL.md 进行多维度质量评分，生成雷达图评分卡，用户可保存/分享。

**评分机制：CC Skill 模式**

评分不由 Muxvo 直接调用 Claude API，而是通过 CC Skill 完成：
1. Muxvo 在用户的 CC 终端中触发 `muxvo-skill-scorer` Skill
2. Skill 读取目标 SKILL.md，在 CC 内完成评分（利用 CC 已有的 API 访问权限）
3. Skill 输出结构化 JSON 到约定路径（`~/Library/Application Support/Muxvo/skill-scores/`）
4. Muxvo 监听输出文件，读取评分结果并渲染评分卡

**优势**：用户零配置（无需单独配 API Key）；Muxvo 自身的评分 Skill 成为 Showcase 的示范案例；API 成本由 CC 现有配额承担。
...
| 实用性 | 25% | 解决真实问题的程度，使用场景是否清晰 |
| 工程质量 | 25% | Prompt 结构、错误处理、边界条件覆盖 |
| 意图清晰度 | 10% | Skill 目标是否明确、解决的问题是否定义清晰、使用者能否快速理解用途 |
| 设计巧妙度 | 10% | Prompt 设计技巧、工具调用编排、工作流步骤设计是否巧妙高效 |
| 文档完善度 | 15% | 说明文档、使用示例、参数说明的完整性 |
| 可复用性 | 15% | 其他用户能否直接使用、参数化程度、通用性 |

**可选：纳入使用数据**
- 评分 Skill 可读取 Muxvo 本地使用统计（调用次数、使用频率等），作为"实用性"评分的辅助参考
- 需用户在评分前授权（弹窗确认："是否将使用数据纳入评分？这可能提高实用性维度的准确度"）
- 未授权则仅基于 SKILL.md 文本评分
...
**评分一致性保障：**
- 评分 Skill 内部使用 `temperature=0`
- 评分结果与 SKILL.md 内容 hash 绑定缓存，内容未变则返回缓存分数
- 使用等级制展示（±5 分的波动不影响等级）
- 记录 `promptVersion`，Prompt 模板变更时旧缓存自动失效

**安全防护（Prompt Injection）：**
- 评分 Skill 的 Prompt 中使用 XML 标签包裹 SKILL.md 内容（`<skill-content>...</skill-content>`），明确指示 AI 将其视为待评估数据而非指令
- 使用 Claude 的结构化输出（JSON schema 约束），确保返回格式固定
- 后处理验证：总分 = 各维度加权平均（容差 ±2），否则标记异常并重新评分

**评分卡 UI：**
- 六维度雷达图（SVG/Canvas 渲染）
- 总分 + 等级标签 + 称号（如 "Workflow Architect"，由评分 Skill 生成）
- 各维度得分 + 评分理由（可折叠）
- 改进建议（默认折叠，2-3 条具体可操作建议）
- 操作按钮：保存为 PNG / 复制到剪贴板 / 生成展示页

**评分 Skill 规格：**
- Skill 名称：`muxvo-skill-scorer`
- 安装方式：Muxvo 首次触发评分时自动安装到 `~/.claude/skills/muxvo-skill-scorer/`
- 输入：目标 Skill 的 SKILL.md 文件路径 + 可选使用统计 JSON
- 输出：结构化 JSON 写入 `~/Library/Application Support/Muxvo/skill-scores/{skill-dir-name}.json`
- 模型：默认使用 CC 当前配置的模型（通常为 Claude Sonnet）
...
- deep link 检测：尝试触发 `muxvo://` 协议，超时 3 秒未响应则判定未安装
...
| 渠道 | 动作 | 内容 |
|------|------|------|
| Twitter/X | 打开浏览器跳转发推页 | 预填文案：`刚给我的 Skill "{name}" 做了 AI 评分，拿到 {grade} {score} 分！→ {商城链接} #ClaudeCode #Muxvo`，用户可修改后发送 |
| LinkedIn | 打开浏览器跳转分享页 | 预填专业措辞文案 + 链接 |
| 微信 | 生成分享图保存到本地 | 竖版图片：评分雷达图 + Skill 名称 + 分数 + 描述 + 商城页面 QR 码 + Muxvo 品牌。用户拖到微信聊天/朋友圈即可 |
| 复制链接 | 一键复制商城 URL | 适用于 Slack、Discord、邮件等任意平台 |
| 复制徽章 | 一键复制 Markdown badge | `[![Muxvo Score: {grade} {score}](https://img.shields.io/badge/Muxvo-{grade}%20{score}-blue)]({商城URL})`，用户粘贴到 GitHub README 或博客 |
| Discord | 复制格式化文本到剪贴板 | Markdown 格式：`**{name}** · {grade} {score} 分 → {链接}`，适合直接粘贴到 Discord channel |
| Reddit | 打开浏览器跳转发帖页 | 预填标题 + 正文（Markdown 格式），推荐发到 r/ClaudeAI 或 r/ChatGPTCoding |

- 分享面板同时展示在发布成功弹窗和商城详情页的「分享」按钮中
- 根据系统语言自动切换中/英文案模板
- 微信分享图尺寸：750x1334px（竖版，适合手机浏览）
- OG Card 图片尺寸：1200x630px（横版，适合 Twitter/LinkedIn 预览）
...
**嵌入式评分徽章：**
- 使用 shields.io 兼容格式，零维护成本
- 等级对应颜色：Promising=gray, Solid=green, Advanced=blue, Expert=purple, Masterwork=gold
- 点击徽章跳转到商城详情页
...
│  │ 来源     │                                         │   │
│  │ ────     │  ┌────────────────────────────────────┐ │   │
│  │ ☑ 全部   │  │⚡ commit-helper      🏛 官方 [安装] │ │   │
│  │ ☑ 🏛官方  │  │根据 git diff 自动生成 commit msg    │ │   │
│  │ ☑ 📦SMP  │  │来源: Anthropic · by @anthropic      │ │   │
│  │ ☑ 🐙GH   │  ├────────────────────────────────────┤ │   │
│  │ ☑ 💻本地  │  │⚡ tdd-workflow    📦 SkillsMP [安装]│ │   │
│  │          │  │TDD 驱动开发工作流                    │ │   │
│  │ 排序     │  │来源: SkillsMP · by @dev123          │ │   │
│  │ ────     │  ├────────────────────────────────────┤ │   │
│  │ ◉ 来源优先│  │⚡ code-reviewer  🐙 GitHub [已安装✓]│ │   │
│  │ ○ 名称   │  │Review code for quality              │ │   │
...
| CC 终端未运行（AI 评分） | 提示启动 CC | "请先启动一个 Claude Code 终端" |
| 评分 Skill 未安装（AI 评分） | 自动安装 muxvo-skill-scorer | 显示安装进度，安装完成后自动继续评分 |
| 评分 Skill 运行失败（AI 评分） | 最多重试 3 次，仍失败则提示 | "评分失败，请检查网络连接后重试" |
| 评分结果 JSON 解析失败（AI 评分） | 重新评分 1 次，仍失败则提示 | "评分结果格式异常，已自动重试" |
...
| 发布安全检查不通过 | 阻止发布，标红提示具体文件和行号 | "检测到敏感信息（第 N 行），请移除后重试" |
| 名称已被占用（发布） | 提示冲突，建议改名 | "商城中已有同名 Skill，请更换名称或添加前缀" |
| 网络中断（发布中） | 保存草稿到本地 | "网络中断，草稿已保存。网络恢复后可在 Skills 列表继续发布" |
| 微信分享图生成失败 | 回退为复制链接 | "分享图生成失败，已复制链接到剪贴板" |
| Deep Link 未响应（Web 页面安装） | 3 秒后自动展开手动安装选项 | "未检测到 Muxvo，请选择其他安装方式" |
| 对话式发布 GitHub 未授权 | 提示在 Muxvo GUI 中登录 | "请先在 Muxvo 中登录 GitHub 账号" |
...
- AI 评分通过内置 CC Skill（`muxvo-skill-scorer`）完成，Muxvo 触发 CC 运行评分 Skill
- 利用 CC 已有的 API 访问权限，用户无需额外配置 API Key
- 评分结果缓存在本地 `~/Library/Application Support/Muxvo/skill-scores/`
- 零服务端成本（API 费用由 CC 现有配额承担）
...
| 事件名 | 触发时机 | 携带参数 |
|--------|----------|----------|
| `skill.search` | 在聚合浏览器中搜索 | `{query, result_count, sources_searched}` |
| `skill.detail_view` | 查看 Skill 详情页 | `{package_name, source}` |
...
| `score.result` | 评分完成 | `{skill_name, total_score, grade, duration_ms}` |
| `score.fail` | 评分失败 | `{skill_name, error_type: "cc_not_running"\|"skill_not_installed"\|"run_fail"\|"json_parse"}` |
...
- [ ] muxvo-skill-scorer Skill 设计与开发（Prompt 模板、输入/输出格式、一致性验证——同 Skill 多次评分偏差 < 5 分）
- [ ] 评分 Skill 与 Muxvo 的通信机制确认（文件监听 vs CLI 输出解析）
- [ ] Skill Showcase 展示页模板设计（2-3 套主题视觉设计）
- [ ] 展示页 GitHub Pages 自动发布流程验证（GitHub API + Pages 配置）
- [ ] 评分卡六维度雷达图渲染方案选型（SVG vs Canvas vs ECharts）
- [ ] muxvo-publisher Plugin 与 Muxvo Electron 的 Token 传递机制验证（IPC / 临时文件 / 环境变量）
- [ ] CC /plugin marketplace 自定义源功能验证——确认 CC 是否支持 `/plugin marketplace add muxvo`，如不支持则安装策略回退为纯文件操作 + Deep Link
- [ ] 42plugin API 公开可用性验证（聚合源可行性）
...
- 实现 AI Skill 评分功能（通过内置 CC Skill），邀请 20-30 个 CC 活跃用户内测
- 重点观察：用户是否愿意打分？评分卡是否被分享到社交媒体？
- 根据反馈调整评分维度和 Prompt
- **Go/No-Go 门槛**：内测用户中评分卡分享率 < 15%（即 30 人中少于 5 人分享）则暂停 P2 开发，重新评估策略方向
