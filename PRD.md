...
| 版本号 | V1.4 |
| 创建人 | RL |
| 修订记录 | V1.0 首版；V1.1 新增 Skills/Hooks 社区市场功能（N-Z）；V1.2 基于辩证分析优化：明确产品阶段、重建优先级、补充后端架构/安全模型/跨平台策略；V1.3 基于三人团队评审落地：产品定位通用化（支持任意 AI CLI 工具）、进程状态检测机制、cwd 切换修正、Esc 焦点规则、tile 双击聚焦、首次引导、缺省态、数据埋点、聊天导出；V1.4 新增富编辑器覆盖层（V1，用 Web 富编辑器替代终端原生输入，支持图片粘贴和自然文本编辑）、V2 商城策略调整为多源聚合+上下文推荐（不自建商城，聚合 Anthropic 官方/SkillsMP/GitHub 等已有生态） |
| 评审时间 | 待评审 |
| 评审结果 | 待评审 |
...
- **对 CCMux 产品**：上下文感知智能构成护城河——CCMux 包裹着用户的终端，能感知当前项目上下文并智能推荐 skill，这是 Web 商城和 CLI 工具都无法复制的能力
...
| 可复制 | 不可复制 |
|--------|----------|
| UI 代码、聚合逻辑 | 上下文感知推荐（知道用户此刻需要什么 skill） |
| 安装/发布逻辑 | 用户行为数据（skill 使用频率、项目关联） |
| 数据库结构 | 富编辑器 + 终端的深度集成体验 |
| | 多源聚合的覆盖广度和更新速度 |
| | 用户习惯和工作流锁定 |

**数据飞轮**：更多用户使用 → 更多行为数据 → 更精准的上下文推荐 → 更高的 skill 安装率 → 更多用户留存
...
> **策略调整说明**：截至 2026 年 2 月，Anthropic 已内建 `/plugin marketplace` CLI 命令，SkillsMP 已索引 87,000+ skills，SkillHub 有 7,000+ 评估过的 skills。自建商城无法与平台方和先发第三方竞争。CCMux 的独特优势是包裹着用户终端、能感知工作上下文，因此 V2 策略调整为**多源聚合 + 上下文推荐**——不做内容，做智能。
...
| P2 | 自有发布/分享 | 仅在聚合+推荐模式验证后考虑。GitHub OAuth 登录 + 从本地 skill 发布到 CCMux 自有源 | V2-P2 | 痛点5 |
...
- 发布到 CCMux 自有源（需先实现自有服务端）

### 3.9 本地注册表与使用分析

CCMux 在本地记录包安装信息和使用分析数据：
...
| 使用 | 在 Claude Code 中正常使用新 skill | skill 自动生效，CCMux 记录使用数据 | 提升工作效率 |
...
|------|------|
| 无附件 | 编辑器正常文本模式 |
| 附加中 | 用户粘贴图片，保存到 `/tmp/ccmux-images/{uuid}.png` |
...
> 仅在聚合+推荐模式验证后考虑实现。需先搭建 CCMux 自有服务端。
>
> 基本流程：从配置管理器选择本地 skill → 自动读取元数据 → 发布到 CCMux 自有源。详细规格在 V2-P2 启动时编写。

### 8.9 上下文推荐系统（功能 CR，V2-P1）

**推荐原理：**
CCMux 包裹着用户的终端，能感知当前项目的上下文信息，据此从聚合源中推荐最相关的 skill。
...
> 聚合模式下 CCMux 不托管内容，因此不需要发布审查和内容治理。各聚合源自行负责内容质量。CCMux 仅在 Hook 安装时提供本地安全审查（见 8.11）。
>
...
3. 图片保存到临时目录：`/tmp/ccmux-images/{uuid}.png`（Electron `app.getPath('temp')`）
4. 发送时按前台工具适配：
...
```
┌────────────────────────────────────────────────────────────┐
│  CCMux          [终端 3] [Skills 28] [MCP 3] [Skills]      │ ← 菜单栏
├────────────────────────────────────────────────────────────┤
│                                                            │
│  ┌──────────┬─────────────────────────────────────────┐   │
│  │          │  🔍 搜索所有来源的 skills...             │   │
...
聚合模式下，CCMux 客户端直接请求各公开数据源的 API：
