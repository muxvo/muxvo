...
| 版本号 | V1.6 |
| 创建人 | RL |
| 修订记录 | V1.0 首版；V1.1 新增 Skills/Hooks 社区市场功能（N-Z）；V1.2 基于辩证分析优化：明确产品阶段、重建优先级、补充后端架构/安全模型/跨平台策略；V1.3 基于三人团队评审落地：产品定位通用化（支持任意 AI CLI 工具）、进程状态检测机制、cwd 切换修正、Esc 焦点规则、tile 双击聚焦、首次引导、缺省态、数据埋点、聊天导出；V1.4 新增富编辑器覆盖层（V1，用 Web 富编辑器替代终端原生输入，支持图片粘贴和自然文本编辑）、V2 商城策略调整为多源聚合+上下文推荐（不自建商城，聚合 Anthropic 官方/SkillsMP/GitHub 等已有生态）；V1.5 护城河策略重构（三层递进：匿名集体智慧→人际连接→AI 效果验证层）、V1 数据采集补充；V1.6 新增 Skill 宣传页功能（V2-P2，与 GitHub OAuth 同期上线），自动生成精美 Skill 展示页+一键在线发布+个人技能作品集；帮建社区暂不纳入版本计划，设定社区功能准入条件；评审修复：SP 优先级对齐、补充外部访问者转化路径/数据结构/流程图/状态机/缺省态/异常处理/埋点事件，修正 OAuth PKCE 流程，统一 marketplace.json 结构，修复 Cmd+M 快捷键冲突 |
| 评审时间 | 待评审 |
| 评审结果 | 待评审 |
...
| 宣传页模板和生成逻辑 | 用户在 Muxvo 上积累的 Skill 作品集和外部分享链接网络（迁移成本高） |
...
> V1.6 补充：Skill 宣传页通过外部分享链接引入新用户（宣传页访问 → 下载 Muxvo → 产生使用数据），形成飞轮的外部增长入口。个人技能主页（PP）的作品集积累则提高了用户迁移成本，强化锁定效应。
...
- V2 启动匿名聚合前，必须：(1) 发布隐私政策，明确说明采集范围和用途；(2) 用户主动 opt-in 同意数据共享；(3) 数据脱敏——移除项目路径、文件名、代码内容等可识别信息，仅保留统计特征（语言、框架、Skill 使用频率等）
- 用户可随时在 Settings 中关闭数据共享或删除已上传数据
...
| SP | Skill 宣传页 | 用户发布 skill 时自动生成精美宣传页，展示 skill 能力、使用说明、作者信息。支持一键发布到 GitHub Pages 生成可分享链接，可贴到简历/社交媒体。后续迭代为个人技能作品集 | V2-P2 | 痛点5 |
...
| PP | 个人技能主页 | 汇聚用户发布的所有 Skill 为作品集页面，形成 AI 技能简历，可对外分享用于求职/接单 | V2-P3 | 痛点5 |
...
- `{project-hash}/`：项目路径的 hash 值作为目录名，避免路径中的特殊字符问题
- 遇到缺失必要字段：该条记录跳过，不崩溃
...
### 3.9 Skill 宣传页策略（V2-P2）

> Skill 宣传页是创作者激励和产品外部传播的核心功能，与 GitHub OAuth（V2-P2）同期上线。

**核心理念**：用户创建/发布 Skill 后，自动生成精美的单页展示页面，解决两个问题：
1. **创作者激励**：给 Skill 作者"发布的额外理由"——不仅有人用，还能展示自己
2. **外部获客**：每个可分享链接都是 Muxvo 的免费获客渠道

**信息提取**（自动从 SKILL.md 解析）：
- Skill 名称、描述（YAML frontmatter `name`/`description` 字段）
- 功能要点（Markdown body 中的列表项）
- 使用说明（Instructions 段落）
- 适用场景（`description` 中的 "Use when" 模式）
...
**页面结构**：
```
├── Hero Section（Skill 名称 + 一句话描述 + 安装按钮）
├── 功能要点（自动提取）
├── 效果演示（作者上传截图/GIF，或示例代码块）
├── 使用说明（自动提取）
├── 作者信息（GitHub 头像 + bio）
└── 安装/引用（安装命令 + 来源链接）
```

**模板主题**：提供 3-5 套预设模板，作者可选择。颜色根据 Skill 分类自动分配主色调。

**托管方案（渐进式）**：
1. **V2-Phase 2（MVP）**：通过 GitHub OAuth + GitHub Pages 一键发布，URL 格式 `username.github.io/muxvo-skills/skill-name`
2. **长期**：迁移到 Muxvo 自有域名 `skill.muxvo.com/@username/skill-name`

**个人技能主页（V2-Phase 3）**：汇聚用户所有 Skill 为作品集页面，形成"AI 技能简历"，可对外分享用于求职/接单。
...
| 宣传页 | 预览自动生成的宣传页，可选择模板主题、补充截图 | 实时预览宣传页效果 | 美化展示 |
| 发布 | 点击 [发布] | 上传到 Muxvo 自有源 + 一键发布宣传页到 GitHub Pages | 分享给社区 |
| 分享 | 复制宣传页链接，贴到简历/社交媒体 | 提供一键复制链接按钮 | 对外展示 |

### 4.4 宣传页浏览者旅程（V2-P2）

> 外部用户通过社交媒体/简历中的宣传页链接进入，是 Muxvo 免费获客的核心转化路径。

| 阶段 | 用户动作 | 系统反馈 | 关键目标 |
|------|----------|----------|----------|
| 发现 | 在社交媒体/简历/搜索引擎中看到宣传页链接 | - | 触达潜在用户 |
| 浏览 | 点击链接打开宣传页 | 展示 Skill 名称、功能要点、效果演示、作者信息 | 了解 Skill 价值 |
| 安装意向 | 点击「安装此 Skill」按钮 | 已安装 Muxvo → `muxvo://install/skill-name` deep link 跳转安装；未安装 → 显示手动安装命令 + Muxvo 下载引导 | 转化为安装/下载 |
| 探索 | 浏览页面底部「更多 Skill」和 Muxvo 介绍 | 展示 Muxvo 品牌标识、下载链接、其他热门 Skill | 提升品牌认知 |
...
### 5.10 Skill 宣传页生成与发布流程（V2-P2）

```mermaid
flowchart TD
    Start([用户在 Skills 列表选中本地 Skill]) --> Parse[解析 SKILL.md]
    Parse --> HasFrontmatter{有 YAML frontmatter?}
    HasFrontmatter -->|是| ExtractAuto[自动提取 name/description/features]
    HasFrontmatter -->|否| ExtractFallback[降级提取：文件名→name，首段→description]
    ExtractAuto --> GenerateDraft[生成宣传页草稿]
    ExtractFallback --> GenerateDraft
    GenerateDraft --> Preview[预览宣传页]
    Preview --> Edit{用户编辑?}
    Edit -->|选择模板| SelectTemplate[切换模板主题]
    Edit -->|补充截图| AddScreenshots[上传截图/GIF]
    Edit -->|修改文案| EditContent[编辑标题/描述/标签]
    SelectTemplate --> Preview
    AddScreenshots --> Preview
    EditContent --> Preview
    Edit -->|满意| CheckAuth{已登录 GitHub?}
    CheckAuth -->|否| LoginGitHub[GitHub OAuth 登录]
    LoginGitHub --> CheckAuth
    CheckAuth -->|是| Publish[发布到 GitHub Pages]
    Publish --> CheckRepo{muxvo-skills 仓库存在?}
    CheckRepo -->|否| CreateRepo[通过 GitHub API 自动创建仓库]
    CreateRepo --> PushPages[推送 HTML 到仓库 + 启用 Pages]
    CheckRepo -->|是| PushPages
    PushPages --> Success([发布成功，显示可分享链接])
    Publish -->|失败| Error[显示错误信息 + 重试按钮]
```
...
```mermaid
stateDiagram-v2
    [*] --> NotGenerated: Skill 存在但未生成宣传页

    NotGenerated --> Generating: 点击「生成宣传页」
    Generating --> Previewing: 解析完成，生成草稿
    Generating --> GenerateFailed: SKILL.md 解析失败

    GenerateFailed --> Generating: 重试
    GenerateFailed --> NotGenerated: 取消

    Previewing --> Editing: 用户修改模板/截图/文案
    Editing --> Previewing: 保存修改，刷新预览

    Previewing --> Publishing: 点击「发布宣传页」
...
| 状态 | UI 表现 |
|------|---------|
| NotGenerated | Skills 列表中显示「生成宣传页」按钮 |
| Generating | 按钮变为 loading 状态 + "正在解析 SKILL.md..." |
| GenerateFailed | 弹出错误提示 + 手动填写入口 |
| Previewing | 打开宣传页预览面板（右侧） |
| Editing | 预览面板切换为编辑模式（模板选择器、截图上传、文案编辑） |
...
### 7.7 宣传页配置（SkillPromoPage）

> 存储在本地 `~/.muxvo/promo-pages/{skill-dir-name}.json`，发布后同步到 GitHub Pages。

```json
{
  "version": 1,
  "skillDirName": "commit-helper",
  "skillName": "generating-commit-messages",
  ...
  "extractedFrom": "SKILL.md",
  "lastGeneratedAt": "2026-02-10T00:00:00Z",
  "lastEditedAt": "2026-02-10T00:00:00Z"
}
```

### 7.8 个人技能主页（PersonalPortfolio，V2-P3）

> 存储在本地 `~/.muxvo/portfolio.json`，V2-P3 阶段发布到 `skill.muxvo.com/@username`。
...
```json
{
  ...
  "skills": [
    {
      "skillDirName": "commit-helper",
      "promoPageUrl": "https://rl.github.io/muxvo-skills/commit-helper",
      "featured": true,
      "order": 1
    }
  ],
...
Muxvo 在后台维护 CC 聊天记录的增量镜像，存储在 `~/Library/Application Support/Muxvo/chat-history/`（参见 3.5 节数据目录结构）。
...
**同步逻辑：**
- 按 sessionId 去重
- 用文件 mtime 判断是否需要更新（CC 文件更新 → 覆盖镜像副本）
...
| 元素 | 数据来源 | 说明 |
|------|----------|------|
| 标题 | session 首条 user 消息 | 截取前 30 字符作为标题 |
...
| 代码块 | 语法高亮 + 复制按钮 |
| 工具调用 | 蓝色左边框高亮，显示工具名 + 参数，可折叠/展开 |
| 工具结果 | 绿色左边框高亮，紧跟工具调用下方 |
...
### 8.9 Skill 宣传页（功能 SP，V2-P2）

**功能概述**：用户创建/发布 Skill 后，自动从 SKILL.md 提取信息，生成精美的单页宣传页面，支持在线发布和外部分享。
...
| 使用说明 | Markdown body 中 Instructions 段落 | 中 |
| 适用场景 | `description` 中的 "Use when" 模式匹配 | 中 |
| 复杂度估计 | 文件行数、references/ 目录文件数 | 低 |
...
- 用户可在宣传页编辑界面手动修正所有自动提取的内容

**作者可补充**：效果截图/GIF、分类标签、自定义封面图。

**模板系统**：
- 提供 3-5 套预设模板主题（如 developer-dark、minimal-light、vibrant 等）
- 颜色根据 Skill 分类自动分配主色调（development=蓝、design=紫、workflow=绿）
- 数据徽章：安装量、使用频率（来自使用分析 SA 的数据）

**发布流程**：
1. 用户在配置管理器的 Skills 列表中，对本地 skill 点击「生成宣传页」
2. 系统自动解析 SKILL.md，用选定模板生成预览
3. 用户可选择模板主题、补充截图/标签
4. 点击「发布宣传页」→ 通过 GitHub OAuth + GitHub API 一键发布到 GitHub Pages
5. 生成可分享链接（`username.github.io/muxvo-skills/skill-name`），提供一键复制按钮

**GitHub Pages 发布细节：**
- 所需 OAuth scope：`repo`（创建/写入仓库）、`read:user`（读取用户信息）
- 首次发布时，通过 GitHub API 检查用户是否有 `muxvo-skills` 仓库，无则自动创建（公开仓库 + 启用 GitHub Pages）
- 后续发布直接 push HTML 文件到对应目录
- 如仓库已存在但非 Muxvo 创建，提示用户确认或使用其他仓库名

**安装按钮行为：**
- 已安装 Muxvo：点击触发 `muxvo://install/{skill-source}/{skill-name}` deep link，Muxvo 打开安装确认页
- 未安装 Muxvo：显示两个选项——(1) 手动安装命令（`cp` 到 `~/.claude/skills/`）(2)「下载 Muxvo」按钮（跳转 Muxvo 官网下载页）
- deep link 检测方案：页面加载后尝试触发 `muxvo://` 协议，超时 2 秒未响应则判定为未安装，自动切换到手动安装模式

**页面底部品牌区：**
- Muxvo logo + 一句话介绍（"AI CLI 工具的桌面工作台"）
- 「下载 Muxvo」CTA 按钮
- 「浏览更多 Skill」链接（跳转 Muxvo 官网 Skill 目录页）

**安全注意事项**：
- 宣传页中 Markdown 渲染必须使用 DOMPurify 做 sanitize，防范 XSS
- 不允许在宣传页中嵌入任意 HTML/JavaScript
- 图片只允许从白名单域名加载（GitHub、Imgur 等）

**后续迭代（V2-Phase 3）**：
- 个人技能主页：汇聚用户所有 Skill 的作品集页面（`skill.muxvo.com/@username`）
- 安装量/使用数据实时展示
- 高级展示模板（付费功能）

### 8.10 上下文推荐系统（功能 CR，V2-P1）
...
| SKILL.md 解析失败（宣传页生成） | 切换为手动填写模式，用文件名作 name | 提供 SKILL.md 格式规范链接 |
| GitHub Pages 发布超时（宣传页发布） | 30秒超时后提示重试 | 保存草稿到本地，可稍后重试 |
| GitHub API rate limit（宣传页发布） | 提示"GitHub API 配额已用完，请稍后再试" | 显示配额重置时间 |
| 宣传页图片上传失败（截图补充） | 提示文件大小限制（单张 ≤ 5MB）和格式限制（PNG/JPG/GIF） | 自动压缩重试 |
| GitHub 仓库被手动删除（已发布宣传页） | 检测到仓库不存在，状态标记为 Unpublished | 提示用户重新发布 |
...
| 场景 | 缺省态展示 |
|------|-----------|
| 无本地 Skill | "还没有本地 Skill。创建一个 Skill 后即可生成宣传页。[如何创建 Skill]" |
| SKILL.md 解析失败（无 frontmatter） | "无法自动提取信息，请手动填写 Skill 名称和描述。[手动填写]" |
| GitHub OAuth 未登录 | "发布宣传页需要 GitHub 账号。[登录 GitHub] 或 [导出为本地 HTML]" |
| GitHub Pages 发布失败 | "发布失败：{错误原因}。[重试] [导出为本地 HTML]" |
| 宣传页 CDN 未生效 | "宣传页已发布，GitHub Pages 通常需要 1-2 分钟生效。[刷新检查]" |
...
| 宣传页生成率 | 生成宣传页的 Skill 数 / 本地 Skill 总数 | 衡量宣传页功能的采用度 |
| 宣传页发布率 | 发布数 / 生成数 | 衡量从生成到发布的转化 |
| 分享链接点击量 | 宣传页外部访问次数（需页面端统计） | 衡量外部传播效果 |
| 外部转化率 | 宣传页访问后下载 Muxvo 的比例 | 衡量获客效果 |
...
> **关于商业化**：V1 阶段不考虑商业化，专注验证产品价值和获取用户。V2 阶段根据用户基数和生态成熟度，逐步引入以下商业化方向：
>
> **潜在收入来源（V2+）：**
> | 收入来源 | 描述 | 启动条件 |
> |---------|------|---------|
> | 宣传页高级模板 | 3-5 套免费基础模板 + 付费高级模板（自定义布局、动效、品牌色等） | SP 功能上线且生成率 > 30% |
> | 个人技能主页 Pro | 免费基础主页 + Pro 版（自定义域名、SEO 优化、数据分析面板） | PP 功能上线且月活作者 > 500 |
> | 团队/企业版 | 团队共享 Skill 库、统一配置管理、使用分析仪表盘 | 企业用户需求验证 |
> | 推荐位 | Skill 聚合浏览器中的付费推荐位（类似应用商店广告） | 聚合浏览器月活 > 5000 |
>
> **免费/付费边界原则**：核心终端工作台功能永久免费，付费功能围绕"展示增强"和"团队协作"，不限制个人用户的基础使用。
...
| 事件名 | 触发时机 | 携带参数 |
|--------|----------|----------|
| `promo.generate` | 生成宣传页草稿 | `{skill_name, has_frontmatter: boolean, extract_quality: "full"\|"partial"\|"fallback"}` |
| `promo.template_switch` | 切换宣传页模板 | `{from_template, to_template}` |
| `promo.screenshot_add` | 添加截图/GIF | `{file_type, file_size_kb}` |
| `promo.publish` | 发布宣传页到 GitHub Pages | `{skill_name, template, has_screenshots: boolean, publish_duration_ms}` |
| `promo.publish_fail` | 发布失败 | `{skill_name, error_type: "auth"\|"network"\|"rate_limit"\|"repo_create"}` |
| `promo.share_link_copy` | 复制宣传页分享链接 | `{skill_name, context: "publish_success"\|"skill_list"\|"portfolio"}` |
| `promo.update` | 更新已发布的宣传页 | `{skill_name, changed_fields: string[]}` |
...
- [ ] 上下文推荐算法的项目特征提取准确度验证
- [ ] Skill 宣传页模板设计（3-5 套主题视觉设计）
- [ ] 宣传页 GitHub Pages 自动发布流程验证（GitHub API + Pages 配置）
- [x] 帮建社区功能评审 → 暂不纳入版本计划，先用 GitHub Discussions 零成本验证需求。准入条件：月活 > 5000、悬赏板月活帖 > 100、自有发布月新增 Skill > 50
...
**社区功能准入条件（帮建社区/讨论区等）：**
> V1.6 评审结论：帮建社区想法暂不纳入版本计划。以下条件全部满足前，不启动任何社区功能开发。过渡期用 GitHub Discussions 零成本验证需求。

- 月活用户 > 5,000
- 需求悬赏板（RB）月活帖子 > 100
- 自有发布（P2）月新增 Skill > 50
...
| V2-Phase 2 | 上下文推荐(CR) + 更新检测(T) + 富编辑器高级(RE3) + **Skill 宣传页(SP)** | 智能推荐验证 + Skill 展示传播 |
| V2-Phase 3 | 自有发布(P2) + 需求悬赏板(RB)（如需求验证通过）+ 个人技能主页(PP) | 社区功能（需服务端） |
