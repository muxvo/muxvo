...
**Skills 安装（来源：聚合源）：**
- 优先委托 CC 终端执行 `/plugin install {name}`，由 CC 原生流程完成安装
- CC 终端不可用时（未运行或无响应），回退到直接文件操作：下载 → 解压到 `~/.claude/skills/{name}/` → 完成
...
```
上传：作者本地 Skill → 安全检查 → AI 评分 → 填写详情 → 发布到商城
分享：商城 → 一键分享到 Twitter/LinkedIn/微信/README 徽章
下载：社交平台/商城 → 一键安装到本地
循环：用得好 → 也来上传 → 飞轮转动
```
...
- 展示页生成后，自动发布到 Muxvo 商城（聚合浏览器可见）
- 发布成功后弹出分享面板：一键分享到 Twitter/LinkedIn/微信/复制链接/复制 README 徽章
- 分享链接指向展示页（V2-P2 阶段为 GitHub Pages，V2-P3 后切换到 showcase.muxvo.com）
- 微信场景生成专属分享图（含评分雷达图 + 二维码），保存到本地供用户直接拖入聊天
...
V2-P0/P1 阶段分享链接指向 GitHub Pages 展示页。V2-P3 上线后统一指向 showcase.muxvo.com。

**聚合浏览器数据源：**

| 源 | 说明 | 标识 |
|----|------|------|
| Muxvo 商城 | 用户通过 Muxvo 发布的 Skill/Plugin | 🏪 Muxvo |
| Anthropic 官方 | CC 官方 Plugin Marketplace | 🏛 官方 |
| 42plugin | 第三方插件生态（如有公开数据） | 🌊 42plugin |
| GitHub 搜索 | 含 SKILL.md 或 plugin.json 的开源仓库 | 🐙 GitHub |
| SkillsMP | agentskills.io 生态 | 📦 SkillsMP |

展示时标注来源，让用户知道 Skill 来自哪里。
...
flowchart TD
    A[用户点击「安装」] --> B{包类型?}
    B -->|纯 Skill| C{CC 终端运行中?}
    B -->|含 Hook/代码| D[弹出安全审查对话框]
    D --> E{用户确认?}
    E -->|取消| F[取消安装]
    E -->|确认| C
    C -->|是| G["向 CC 发送 /plugin install 命令"]
    C -->|否| H[直接文件操作：复制到 ~/.claude/skills/]
    G --> I[安装完成]
    H --> I
    I --> J[写入本地注册表]
    J --> K["按钮变为「已安装 ✓」"]
...
| Anthropic 官方 | CC 官方 Plugin Marketplace | 🏛 官方 |
| 42plugin | 第三方插件生态（如有公开 API） | 🌊 42plugin |
| GitHub 搜索 | 含 SKILL.md 或 plugin.json 的开源仓库 | 🐙 GitHub |
